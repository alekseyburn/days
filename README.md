# Стартовый проект с gulp ![Test Status](https://travis-ci.com/alekseyburn/days.svg?branch=master) [![devDependencies Status](https://david-dm.org/alekseyburn/days/dev-status.svg)](https://david-dm.org/alekseyburn/days?type=dev) [![dependencies Status](https://david-dm.org/alekseyburn/days/status.svg)](https://david-dm.org/alekseyburn/days) 
...
11. При необходимости запустить `npm run critical` для добавления критического css на страницы html.
...
node createBlock.js demo-block        # создаст папку блока, demo-block.scss, подпапка img/ для этого блока
node createBlock.js demo-block pug js # создаст папку блока, demo-block.scss, demo-block.pug, demo-block.js, подпапка img/ для этого блока
```
...
1. Поместить все картинки `{jpg,png,jpg,svg}` в папку `src/img/`.
2. Запустить задачу `npm run img:opt`

Обработчик оптимизирует все изображения, создаст webp для jpeg и png. После выполнения задачи раскидать полученные изображения по необходимым папкам.
